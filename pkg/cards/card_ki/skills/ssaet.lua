...
cardSkill:addEffect("cardskill", {
  ...
  target_filter = function(self, player, to_select, selected, _, card, extra_data)
    ...
    if  not ( (extra_data and extra_data.bypass_distances) or self:withinDistanceLimit(player, true, card, to_select)) then return end
        local max_target_num = self:getMaxTargetNum(player, card)

        if extra_data then  --不占目幖數?目幖上限?
          if extra_data.extra_target_number then max_target_num=max_target_num+extra_data.extra_target_number end
          if extra_data.extra_targets then
    ...
    -- if not  self:modTargetFilter(player, to_select, selected, card, extra_data) then return end --攻程--CardTargetFilter已含  --CardTargetFilter不攷慮次數 prohibitUse
    if   not
      (--次數
