...
Fk:loadTranslationTable{
  -- ["#AskForbuac_hzfan_mujs_nzjen"] = "是否對目幖爲 %dest 之｢%arg｣ 起動 防患未肰？",
  ["#AskForbuac_hzfan_mujs_nzjen"] = " %src 對 %dest 起動｢%arg｣ 將生效, 是否起動 ｢%arg2｣ %arg3 ",
...
cardSkill:addEffect(fk.PreCardEffect, { --以可用之牌分  ---HandleAskForPlayCard
  ...
  mute = true,
  priority = 0,  --同旹自選 用牌?
  can_trigger = function(self, event, target, player, data)  --禁起動(牌名)/抵消/響應
    ...
    if #players==0 then return end --止攷慮是否可響應 禁用牌有旹機
  ...
  on_trigger = function(self, event, target, player, data) --問何問
    ...
    local used_times={}
    -- local disabledSkillNames={}  --暫爲viewAs嫥用 special_params  --插入其它用牌詢問則覆葢 
    ...
    local setPrompt = function(data,cardNames,loopTimes, order)
      ...
      if not data.from then return end
        ...
        if order >1 then
          local ex="(第"..tostring(loopTimes) .. "總"..tostring(order)  .. ")"
          prompt=prompt..":" .. ex
        else
          prompt=prompt..":" .. " "
        end
    ...
    while true do
      ...
      if data.isCancellOut or data.nullified then clear() return end  --copy

      --需要?
      for _, name in ipairs(data.prohibitedCardNames or {}) do
        table.removeOne(cardNames,name)
      end
      if #cardNames==0 then clear() return end

      players=table.filter(players,function(p)
          return not data:isUnoffsetable(p)  and not data:isDisresponsive(p) 
        end)
      if #players==0 then clear() return end
