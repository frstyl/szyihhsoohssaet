...
extension:loadSkillSkelsByPath("./packages/szyihhsoohssaet/pkg/cards/card_derive/skills")
...
---------------------------------------------------------------------------------------------
-- local ljenqtszuo__kiuc_szjih_sje_ttiac = fk.CreateCard{
--   name = "ljenqtszuo__kiuc_szjih_sje_ttiac",
--   type = Card.TypeBasic,
--   skill = "ljenqtszuo__kiuc_szjih_sje_ttiac_skill",
--   is_damage_card=true,
...
-------------------------------------------------------------------------------------------------裝僃
local moucqtszhioc_hzaach = fk.CreateCard{
  name = "&moucqtszhioc_hzaach",
  type = Card.TypeEquip,
  sub_type = Card.SubtypeTreasure,
  -- equip_skill = "#moucqtszhioc_hzaach_skill",
...
local phaavshsfec_phaavs = fk.CreateCard{
  name = "&phaavshsfec_phaavs",
  type = Card.TypeEquip,
  sub_type = Card.SubtypeWeapon,
  equip_skill = "#phaavshsfec_phaavs_skill",
  skill = "self_equip_skill",
  attack_range = 6,  --技能
  dynamic_attack_range = function(self, player)  --技能 --card效果不一應書于player
    if player then
      local mark = player:getTableMark("@phaavshsfec_phaavs")
      return #mark == 2 and mark[2] or nil
    end
  end,
}
...
--
local gracqgi_gi = fk.CreateCard{
  name = "&gracqgi_gi",
  type = Card.TypeEquip,
  sub_type = Card.SubtypeTreasure,
  equip_skill= "#gracqgi_gi_skill",
  skill = "self_equip_skill",
}
extension:loadCardSkels {gracqgi_gi,}
...
local noaqmjens_mjens = fk.CreateCard{
  name = "&noaqmjens_mjens",
  type = Card.TypeEquip,
  sub_type = Card.SubtypeTreasure,
  -- equip_skill = "#noaqmjens_mjens_skill",
  skill = "self_equip_skill",
  -- attack_range = 6,  --技能
  dynamic_equip_skills = function(self, player)  --self card
    --movecard 171
    local card= self:isConverted() and Fk:getCardById(self.subcards[1]) or self
    if card:getMark("@noaqmjens_mjens")~=0 then  
      local t={}
      for _, name in  ipairs(card:getMark("@noaqmjens_mjens") ) do
        table.insert(t,Fk.skills[name])
      end
       return t
    end
  end,
