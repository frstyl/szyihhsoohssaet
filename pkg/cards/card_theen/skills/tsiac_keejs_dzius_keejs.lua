...
Fk:loadTranslationTable{
  ...
  ["#tsiac_keejs_dzius_keejs-use_to"] = "%desc 對%src 起動%arg，是否起動【將計就計】抵消 取得%arg？",
  ["#tsiac_keejs_dzius_keejs-use"] = "%desc  起動%arg，是否起動【將計就計】抵消 取得%arg？",
}
...
cardSkill:addEffect("cardskill", {
  ...
  on_effect = function(self, room, effect)
    if effect.responseToEvent then
      ...
      effect.responseToEvent.extra_data.tsiac_keejs_dzius_keejs =effect.from.id --一次起動事件止有一將計就計
    end
  end,
})
