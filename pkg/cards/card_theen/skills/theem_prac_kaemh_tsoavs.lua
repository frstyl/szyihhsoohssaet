...
Fk:loadTranslationTable{
  ...
  ["#theem_prac_kaemh_tsoavs-use"] = "%src 對伱起動殺誘  是否起動 添兵減竈",
  ["#theem_prac_kaemh_tsoavs-ssaet"] = "添兵減竈 對 %src 起動殺",
  ["#theem_prac_kaemh_tsoavs_delay"] = "添兵減竈 效果, %tos 需對 %from 起動殺 "
}
...
cardSkill:addEffect("cardskill", {
  ...
  on_effect = function(self, room, effect)
    ...
    if not effect.responseToEvent then return end
      ...
      newTarget:drawCards(1,cardSkill.name)  --起動者 目幖?
