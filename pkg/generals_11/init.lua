local extension = Package:new("generals_11")
extension.extensionName = "szyihhsoohssaet"
extension:loadSkillSkelsByPath("./packages/szyihhsoohssaet/pkg/generals_11/skills")

Fk:loadTranslationTable{
["generals_11"] = "101~110",
["tsshioh__kvoan"] = "楚",
}
--
--童 牛
--龔端 龔正
 --黃達
-- 張管營世開 龐元 龐氏 
--范全 方翰
--段二 段五
General:new(extension, "quackhracs", "tsshioh__kvoan", 4):addSkills{"khihprac"}  --,"kaahtsshioh"
Fk:loadTranslationTable{
["quackhracs"] = "王慶",
["#quackhracs"] = "楚王",
["designer:quackhracs"] = "設計",
["cv:quackhracs"] = "配音",
["illustrator:quackhracs"] = "畫師",
["~quackhracs"] = "來世,再不渡淸江",
}

--廖立
-- 丘翔 施俊 劉敏 魯成
--奚勝
General:new(extension, "dvoanssoamnniac", "tsshioh__kvoan", 4):addSkills{"tsjasthoeojs"}  --忘
Fk:loadTranslationTable{
["dvoanssoamnniac"] = "段三",
["#dvoanssoamnniac"] = "大蟲窩",
["designer:dvoanssoamnniac"] = "設計",
["cv:dvoanssoamnniac"] = "配音",
["illustrator:dvoanssoamnniac"] = "畫師",
["~dvoanssoamnniac"] = "伱",
}

General:new(extension, "lihdzsios", "tsshioh__kvoan", 3):addSkills{"tszhyitsjevs","kiamsmuoh"}
Fk:loadTranslationTable{
["lihdzsios"] = "李助",
["#lihdzsios"] = "金劍先生",
["designer:lihdzsios"] = "設計",
["cv:lihdzsios"] = "配音",
["illustrator:lihdzsios"] = "畫師",
["~lihdzsios"] = "金劍落地再不能起",
}

General:new(extension, "hzeejqszics", "tsshioh__kvoan", 4):addSkills{"tszjecsprac","gxeqprac"}
Fk:loadTranslationTable{
["hzeejqszics"] = "奚勝",
["#hzeejqszics"] = "統軍",
["designer:hzeejqszics"] = "設計",
["cv:hzeejqszics"] = "配音",
["illustrator:hzeejqszics"] = "畫師",
["~hzeejqszics"] = "伱",
}

General:new(extension, "khousmjet", "tsshioh__kvoan", 4):addSkills{"hsuohhsvah","zzikkoot"}
Fk:loadTranslationTable{
["khousmjet"] = "寇烕",
["#khousmjet"] = "毒焰鬼王",  --赤焰毒王
["designer:khousmjet"] = "設計",
["cv:khousmjet"] = "配音",
["illustrator:khousmjet"] = "畫師",
["~khousmjet"] = "a",
}

General:new(extension, "doohhzaok", "tsshioh__kvoan", 5):addSkills{"craktszjens","jiacqtshjes"}
Fk:loadTranslationTable{
["doohhzaok"] = "杜壆",
["#doohhzaok"] = "西軍統軍",
["designer:doohhzaok"] = "設計",
["cv:doohhzaok"] = "配音",
["illustrator:doohhzaok"] = "畫師",
["~doohhzaok"] = "雙權難敵四手",
}
--
-- 102. 第一百零一回　謀墳地陰險產逆　蹈春陽妖𧰚生姦
-- 103. 第一百零二回　王慶因姦喫官司　龔端被打師軍犯
-- 104. 第一百零三回　張管營因妾弟喪身　範節級為表兄醫臉
-- 105. 第一百零四回　段家莊重招新女婿　房山寨雙並舊強人

-- 106. 第一百零五回　宋公明避暑療軍兵　喬道淸回風燒賊寇
-- 107. 第一百零六回　書生談笑卻強敵　水軍汨沒破堅城
-- 108. 第一百零七回　宋江大勝紀山軍　朱武打破六花陣
-- 109. 第一百零八回　喬道淸興霧取城　小旋風藏炮擊賊
General:new(extension, "seevqkaaqsyer", "pujh", 5):addSkills{"liuksyer","ddwenqszio"}
Fk:loadTranslationTable{
["seevqkaaqsyer"] = "蕭嘉繐",
["#seevqkaaqsyer"] = "天生嘉禾",
["designer:seevqkaaqsyer"] = "設計",
["cv:seevqkaaqsyer"] = "配音",
["illustrator:seevqkaaqsyer"] = "畫師",
["~seevqkaaqsyer"] = "閒雲野鶴,何天之不可飛",
}
-- 110. 第一百零九回　王慶渡江被捉　宋江剿寇成功
-- 111. 第一百一十回　燕青秋林渡射　宋江東京城獻俘

return extension
