...
cardSkill:addEffect("cardskill", {
  ...
  mod_target_filter = function(self, player, to_select, selected, card, extra_data)
      if 
      to_select ~= player 
      and to_select.hp<player.hp
      then
        local c = Fk:cloneCard("ssaet")
        c:setVSPattern(nil, to_select, ".")
      return #c:getDefaultTarget(to_select)~=0
      end
  end,
  ...
  on_effect = function(self, room, effect)  --生效 選擇-用殺(轉化印牌旹機) 不用殺效果 共3
    ...
    local giveUp = function ()
        ...
        if #to:getCardIds("e")==0 then
          room:loseHp(to, 1, cardSkill.name,effect.from)
          return
        end
            -- choice = "loseHp" 
        -- else
        ...
        if choice=="loseHp" then
            room:loseHp(to, 1, cardSkill.name,from)
        else
            local cid = room:askToChooseCard(from, { target = to, flag = "e", skill_name = cardSkill.name })
            room:obtainCard(from, cid, false, fk.ReasonPrey, to, cardSkill.name)  --proposer=to
        end
    end
