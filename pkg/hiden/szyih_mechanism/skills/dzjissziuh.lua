...
dzjissziuh:addEffect("prohibit", {
  is_prohibited = function(self, from, to, card)
    if from and from:hasMark("@@dzjissziuh")  and card and from ~= to then return true end
      for _, suf in ipairs({"","-round" , "-turn" , "-phase" , "-noclear"} ) do
        if table.contains(from:getTableMark("dzjissziuh_to"..suf),to.id) then return true end
      end
  end,
})
