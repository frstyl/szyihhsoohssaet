...
Fk:loadTranslationTable{
  ...
  ["#hqjin_szjer_ljis_doavs_skill"] = "當1脚色受屬性傷害旹,對其下家或下家使用.傳導傷害",
...
cardSkill:addEffect("cardskill", {
  ...
  on_effect = function(self, room, effect)
    if not effect.extra_data or not effect.extra_data.hqjin_szjer_ljis_doavs or not effect.extra_data.hqjin_szjer_ljis_doavs.damageData then 
      return
    end
    ...
    local damage={
      ...
      damage.extra_data.hqjin_szjer_ljis_doavs.effect_data= effect
    room:damage(damage)
...
cardSkill:addEffect(fk.Damaged, {
  ...
  can_trigger = function(self, event, target, player, data)
    if   target~=player or not  Fk:canChain(data.damageType)   then return end

    if data.extra_data and data.extra_data.hqjin_szjer_ljis_doavs and data.extra_data.hqjin_szjer_ljis_doavs.direction then
      return S.getNextOne(target,data.extra_data.hqjin_szjer_ljis_doavs.direction):hasDelayedTrick("hqjin_szjer_ljis_doavs")
    else
      return S.getNextOne(target,1):hasDelayedTrick("hqjin_szjer_ljis_doavs")
      or 
      S.getNextOne(target,-1):hasDelayedTrick("hqjin_szjer_ljis_doavs")
    end
  end,
  on_trigger = function(self, event, target, player, data)
    ...
    if data.extra_data and data.extra_data.hqjin_szjer_ljis_doavs and data.extra_data.hqjin_szjer_ljis_doavs.direction then
      ...
      exe(S.getNextOne(target,direction),direction)
    else
      local targets ={}
      table.insertIfNeed(targets,S.getNextOne(target))
      table.insertIfNeed(targets,S.getNextOne(target,-1))
      for i, p in ipairs(targets) do
        if p:hasDelayedTrick("hqjin_szjer_ljis_doavs") then
