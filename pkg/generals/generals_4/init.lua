local extension = Package:new("generals_4")
extension.extensionName = "szyihhsoohssaet"
extension:loadSkillSkelsByPath("./packages/szyihhsoohssaet/pkg/generals/generals_4/skills")

Fk:loadTranslationTable{
["generals_4"] = "31-40",
-- ["pujh"] = "匪",
-- ["kvoan"] = "官",
-- ["mjin"] = "民",
-- ["tsiacs"] = "將",
}

--32. 第三十一回　武行者醉打孔亮　錦毛虎義釋宋江
--白虎山
General:new(extension, "khouchmrac", "pujh", 4):addSkills { "zyinqssaavs","khoonstous" }
Fk:loadTranslationTable{
["khouchmrac"] = "孔明",
["#khouchmrac"] = "毛頭星",
["designer:khouchmrac"] = "設計",
["cv:khouchmrac"] = "配音",
["illustrator:khouchmrac"] = "畫師",
["~khouchmrac"] = "哥哥吾看守不周",
}

General:new(extension, "khouchliacs", "pujh", 4):addSkills { "noosssaet","hzvoanqkhoos" }
Fk:loadTranslationTable{
["khouchliacs"] = "孔亮",
["#khouchliacs"] = "獨火星",
["designer:khouchliacs"] = "設計",
["cv:khouchliacs"] = "配音",
["illustrator:khouchliacs"] = "畫師",
["~khouchliacs"] = "我不識水嗚",
}

--淸風寨
General:new(extension, "hqeenqzzyins", "pujh", 5):addSkills { "hsoohseevs","loohdvoat"}
Fk:loadTranslationTable{
["hqeenqzzyins"] = "燕順",
["#hqeenqzzyins"] = "錦毛虎",
["designer:hqeenqzzyins"] = "設計",
["cv:hqeenqzzyins"] = "配音",
["illustrator:hqeenqzzyins"] = "畫師",
["~hqeenqzzyins"] = "虎落平陽被犬欺",
}

General:new(extension, "quacqhqrac", "pujh", 4):addSkills {"hzouhpuat"}--,"hsoavsssik" 
Fk:loadTranslationTable{
["quacqhqrac"] = "王英",
["#quacqhqrac"] = "矮腳虎",
["designer:quacqhqrac"] = "設計",
["cv:quacqhqrac"] = "配音",
["illustrator:quacqhqrac"] = "畫師",
["~quacqhqrac"] = "有已娘子值此一生",
}

General:new(extension, "ddxevstheenqdzziuh", "pujh", 4):addSkills {"dziacstsoak","cooqkou",}--,
Fk:loadTranslationTable{
["ddxevstheenqdzziuh"] = "鄭天壽",
["#ddxevstheenqdzziuh"] = "白面郎君",
["designer:ddxevstheenqdzziuh"] = "設計",
["cv:ddxevstheenqdzziuh"] = "配音",
["illustrator:ddxevstheenqdzziuh"] = "畫師",
["~ddxevstheenqdzziuh"] = "",
}
--33. 第三十二回　宋江夜看小鰲山　花榮大鬧淸風寨

--劉知寨
General:new(extension, "hsfaqqvrac", "kvoan", 4):addSkills { "jjinhkiuc","hqximhquoh" }
Fk:loadTranslationTable{
["hsfaqqvrac"] = "花榮",
["#hsfaqqvrac"] = "小李廣",
["designer:hsfaqqvrac"] = "設計",
["cv:hsfaqqvrac"] = "配音",
["illustrator:hsfaqqvrac"] = "畫師",
["~hsfaqqvrac"] = "一腔義烈元相契,封樹高懸兩命亾",
}

--34. 第三十三回　鎮三山大鬧青州道　霹靂火夜走瓦礫場
General:new(extension, "hzvoacqsjins", "kvoan", 5):addSkills{"thoocshzaat"}  --38
Fk:loadTranslationTable{
["hzvoacqsjins"] = "黃信",
["#hzvoacqsjins"] = "鎮三山",
["designer:hzvoacqsjins"] = "設計",
["cv:hzvoacqsjins"] = "配音",
["illustrator:hzvoacqsjins"] = "畫師",
["~hzvoacqsjins"] = "三山崛起,力不从心",
}


General:new(extension, "dzjinqmrac", "tsiacs", 4):addSkills { "boavsnoos" }
Fk:loadTranslationTable{
["dzjinqmrac"] = "秦明",
["#dzjinqmrac"] = "霹靂火",
["designer:dzjinqmrac"] = "設計",
["cv:dzjinqmrac"] = "配音",
["illustrator:dzjinqmrac"] = "畫師",
["~dzjinqmrac"] = "可憐霹靂火,滅地竟无聲",
}

--35. 第三十四回　石將軍村店寄書　小李廣梁山射雁

General:new(extension, "dzzjekjioch", "tsiacs", 5):addSkills { "hzoavqtooh" }
Fk:loadTranslationTable{
["dzzjekjioch"] = "石勇",
["#dzzjekjioch"] = "石將軍",
["designer:dzzjekjioch"] = "設計",
["cv:dzzjekjioch"] = "配音",
["illustrator:dzzjekjioch"] = "畫師",
["~dzzjekjioch"] = "俺一定會贏回來",
}


General:new(extension, "liohpuac", "tsiacs", 4):addSkills{"ljetkrak"}  --54
Fk:loadTranslationTable{
["liohpuac"] = "呂方",
["#liohpuac"] = "小溫矦",
["designer:liohpuac"] = "設計",
["cv:liohpuac"] = "配音",
["illustrator:liohpuac"] = "畫師",
["~liohpuac"] = "山領險峻用力過猛已",
}


General:new(extension, "kvakszics", "tsiacs", 5):addSkills{"pjecskrak"}  --55
Fk:loadTranslationTable{
["kvakszics"] = "郭勝",
["#kvakszics"] = "賽仁貴",
["designer:kvakszics"] = "設計",
["cv:kvakszics"] = "配音",
["illustrator:kvakszics"] = "畫師",
["~kvakszics"] = "就昰像結束已无",
}


--36. 第三十五回　梁山泊吳用舉戴宗　揭陽嶺宋江逢李俊

General:new(extension, "lihljip", "pujh", 3):addSkills { "miuqdzoeoj","dvoatmracs"}
Fk:loadTranslationTable{
["lihljip"] = "李立",
["#lihljip"] = "催命判官",
["designer:lihljip"] = "設計",
["cv:lihljip"] = "配音",
["illustrator:lihljip"] = "畫師",
["~lihljip"] = "生不帶來死不帶去",
}

General:new(extension, "lihtsyins", "mjin", 3,4):addSkills { "crakliu", "jiacqpoa"}  --deecstsshaek
Fk:loadTranslationTable{
["lihtsyins"] = "李俊",
["#lihtsyins"] = "渾江龍",
["designer:lihtsyins"] = "設計",
["cv:lihtsyins"] = "配音",
["illustrator:lihtsyins"] = "畫師",
["~lihtsyins"] = "兄弟輩僃船出海緟頭再來",
}

General:new(extension, "doucqhquj", "mjin", 3):addSkills { "hsicqloan","zzicqloacs","dzjemqszyih"}  --dzjemqdzoac --忘已 波浪瀾濤 
Fk:loadTranslationTable{
["doucqhquj"] = "童威",
["#doucqhquj"] = "出洞蛟",
["designer:doucqhquj"] = "設計",
["cv:doucqhquj"] = "配音",
["illustrator:doucqhquj"] = "畫師",
["~doucqhquj"] = "止看後浪推歬浪",
}

General:new(extension, "doucqmaach", "mjin", 4):addSkills { "ttiachloak","dzjemqszyih"}  --poaqdoav  thoavqdoav
Fk:loadTranslationTable{
["doucqmaach"] = "童猛",
["#doucqmaach"] = "翻江蜃",
["designer:doucqmaach"] = "設計",
["cv:doucqmaach"] = "配音",
["illustrator:doucqmaach"] = "畫師",
["~doucqmaach"] = "非俺哥兩容身之地",
}

--37. 第三十六回　沒遮攔追趕及時雨　船火兒夜鬧潯陽江

General:new(extension, "miukhzveoc", "tsiacs", 5):addSkills { "muoqtssioh","bjevsgxes"}
Fk:loadTranslationTable{
["miukhzveoc"] = "穆弘",
["#miukhzveoc"] = "沒遮攔",
["designer:miukhzveoc"] = "設計",
["cv:miukhzveoc"] = "配音",
["illustrator:miukhzveoc"] = "畫師",
["~miukhzveoc"] = "弟",
}

General:new(extension, "miukzzyins", "tsiacs", 3):addSkills { "paaskeecs","hzfacsdzoeoj"}
Fk:loadTranslationTable{
["miukzzyins"] = "穆順",
["#miukzzyins"] = "小遮攔",
["designer:miukzzyins"] = "設計",
["cv:miukzzyins"] = "配音",
["illustrator:miukzzyins"] = "畫師",
["~miukzzyins"] = "兄弟離散各奔東西",
}

General:new(extension, "sjetqvrach", "mjin", 4):addSkills { "maescjer"}  --4/5
Fk:loadTranslationTable{
["sjetqvrach"] = "薛永",
["#sjetqvrach"] = "病大蟲",
["designer:sjetqvrach"] = "設計",
["cv:sjetqvrach"] = "配音",
["illustrator:sjetqvrach"] = "畫師",
["~sjetqvrach"] = "遇到高手已",
}

General:new(extension, "ttiacqhzfac", "mjin", 4):addSkills { "kiapliak","hzoonqbuos","ljeqhzoon"}
Fk:loadTranslationTable{
["ttiacqhzfac"] = "張橫",
["#ttiacqhzfac"] = "船火兒",
["designer:ttiacqhzfac"] = "設計",
["cv:ttiacqhzfac"] = "配音",
["illustrator:ttiacqhzfac"] = "畫師",
["~ttiacqhzfac"] = "弟",
}
--38. 第三十七回　及時雨會神行太保　黑旋風展浪裏白條


General:new(extension, "toeojstsooc", "tsiacs", 4):addSkills { "kxippoavs", "pxisthoeoms"}  --20
Fk:loadTranslationTable{
["toeojstsooc"] = "戴宗",
["#toeojstsooc"] = "神行太保",
["designer:toeojstsooc"] = "設計",
["cv:toeojstsooc"] = "配音",
["illustrator:toeojstsooc"] = "畫師",
["~toeojstsooc"] = "消息有誤",
}


General:new(extension, "lihgwi", "tsiacs", 4):addSkills { "ssaetliuk", "theenqhzeec"}  --22 --羅真人
Fk:loadTranslationTable{
["lihgwi"] = "李逵",
["#lihgwi"] = "黑旋風",
["designer:lihgwi"] = "設計",
["cv:lihgwi"] = "配音",
["illustrator:lihgwi"] = "畫師",
["~lihgwi"] = "生旹伏侍哥哥,死了,也是哥哥部下一个小鬼",
}

General:new(extension, "ttiacqzzyins", "mjin", 3):addSkills {"zzyinsszyih", "ljeqhzoon","dzoakhzaamh"}
Fk:loadTranslationTable{
["ttiacqzzyins"] = "張順",
["#ttiacqzzyins"] = "浪裏白條",
["designer:ttiacqzzyins"] = "設計",
["cv:ttiacqzzyins"] = "配音",
["illustrator:ttiacqzzyins"] = "畫師",
["~ttiacqzzyins"] = "江中多濁浪萬里无風平",
}
--39  潯陽樓宋江吟反詩 樑山泊戴宗傳假信

General:new(extension, "seevqnziacs", "mjin", 3):addSkills {"ljimqmoo", "ttaekseec" }  --46
Fk:loadTranslationTable{
["seevqnziacs"] = "蕭讓",
["#seevqnziacs"] = "聖手書生",
["designer:seevqnziacs"] = "設計",
["cv:seevqnziacs"] = "配音",
["illustrator:seevqnziacs"] = "畫師",
["~seevqnziacs"] = "心已无聲,生命可否仿造",
}

General:new(extension, "kximqdoarkeen", "mjin", 3):addSkills {"phuachtszjer", "dziacssjim" }
Fk:loadTranslationTable{
["kximqdoarkeen"] = "金大堅",
["#kximqdoarkeen"] = "玉臂匠",
["designer:kximqdoarkeen"] = "設計",
["cv:kximqdoarkeen"] = "配音",
["illustrator:kximqdoarkeen"] = "畫師",
["~kximqdoarkeen"] = "愧",
}
--黃文燁
General:new(extension, "hvoacqmunqprach", "kvoan", 3):addSkills {"tooshzeen", "kaavssvoa" }
Fk:loadTranslationTable{
["hvoacqmunqprach"] = "黃文炳",
["#hvoacqmunqprach"] = "黃鋒刺",
["designer:hvoacqmunqprach"] = "設計",
["cv:hvoacqmunqprach"] = "配音",
["illustrator:hvoacqmunqprach"] = "畫師",
["~hvoacqmunqprach"] = "小人已知過失止求早死",
}

General:new(extension, "tshoartoeoktsziac", "kvoan", 3):addSkills {"tshjechkeens", "phjenqtheec" }
Fk:loadTranslationTable{
["tshoartoeoktsziac"] = "蔡得章",
["#tshoartoeoktsziac"] = "江州太守",
["designer:tshoartoeoktsziac"] = "設計",
["cv:tshoartoeoktsziac"] = "配音",
["illustrator:tshoartoeoktsziac"] = "畫師",
["~tshoartoeoktsziac"] = "本以爲立下奇功 不想反招此禍事",
}
General:new(extension, "tshoarkrac", "kvoan", 4):addSkills {"kaaqszio","dvoatguan" }--, 
Fk:loadTranslationTable{
["tshoarkrac"] = "蔡京",
["#tshoarkrac"] = "姦相",
["designer:tshoarkrac"] = "設計",
["cv:tshoarkrac"] = "配音",
["illustrator:tshoarkrac"] = "畫師",
["~tshoarkrac"] = "此事",
}
--40 樑山泊漢汉劫法場 白龍廟英雄小聚義

General:new(extension, "sp__tsiachkracs", "mjin", 3):addSkills{"tszjeqkeejs",""}   --"hzfektszuo" tsjecqsvoans  -- hzaeksvoans pvoattszuo
Fk:loadTranslationTable{
["sp__tsiachkracs"] = "蔣敬",
["#sp__tsiachkracs"] = "神算子",
["designer:sp__tsiachkracs"] = "設計",
["cv:sp__tsiachkracs"] = "配音",
["illustrator:sp__tsiachkracs"] = "畫師",
["~sp__tsiachkracs"] = "人算不如天算",
}

General:new(extension, "tsiachkracs", "pujh", 3):addSkills{"lihkhoos","ddiuqtsshaek"}   --"hzfektszuo" tsjecqsvoans  -- hzaeksvoans  
Fk:loadTranslationTable{
["tsiachkracs"] = "蔣敬",
["#tsiachkracs"] = "神算子",
["designer:tsiachkracs"] = "設計",
["cv:tsiachkracs"] = "配音",
["illustrator:tsiachkracs"] = "畫師",
["~tsiachkracs"] = "人算不如天算",
}


General:new(extension, "maahljin", "pujh", 3):addSkills{"leecqdeek", "khaavhtous"}  --67
Fk:loadTranslationTable{
["maahljin"] = "馬麟",
["#maahljin"] = "靈駒子",
["designer:maahljin"] = "設計",
["cv:maahljin"] = "配音",
["illustrator:maahljin"] = "畫師",
["~maahljin"] = "盛世不再,止餘悲歌",
}

local hqouqboeoc =General:new(extension, "hqouqboeoc", "pujh", 5)
hqouqboeoc:addSkills{"ttxenhszjes"}  --67
hqouqboeoc:addRelatedSkill("doeocqpuj")
Fk:loadTranslationTable{
["hqouqboeoc"] = "歐鵬",
["#hqouqboeoc"] = "摩雲金翅",
["designer:hqouqboeoc"] = "設計",
["cv:hqouqboeoc"] = "配音",
["illustrator:hqouqboeoc"] = "畫師",
["~hqouqboeoc"] = "連珠箭",
}

General:new(extension, "doavqtsoocquacs", "pujh", 4):addSkills{"khaavhkouc","maanqlik"}  --67
Fk:loadTranslationTable{
["doavqtsoocquacs"] = "陶宗旺",
["#doavqtsoocquacs"] = "九尾龜",
["designer:doavqtsoocquacs"] = "設計",
["cv:doavqtsoocquacs"] = "配音",
["illustrator:doavqtsoocquacs"] = "畫師",
["~doavqtsoocquacs"] = "弟兄輩快",
}
--41  第四十回 宋江智取無為軍　張順活捉黃文炳

General:new(extension, "hzouqgians", "pujh", 2):addSkills{"pujqtszjim","mxenhlik"}  --71 ""
Fk:loadTranslationTable{
["hzouqgians"] = "侯健",
["#hzouqgians"] = "通臂猿",
["designer:hzouqgians"] = "設計",
["cv:hzouqgians"] = "配音",
["illustrator:hzouqgians"] = "畫師",
["~hzouqgians"] = "戰袍雖新",
}

General:new(extension, "hvoacqmunqqxep", "mjin", 3):addSkills {"siuqdzjecs","dzoeonqsyit","loavqtszih"}
Fk:loadTranslationTable{
["hvoacqmunqqxep"] = "黃文燁",
["#hvoacqmunqqxep"] = "黃佛子",
["designer:hvoacqmunqqxep"] = "設計",
["cv:hvoacqmunqqxep"] = "配音",
["illustrator:hvoacqmunqqxep"] = "畫師",
["~hvoacqmunqqxep"] = "~",
}
return extension
