...
phaavshsfec:addEffect(fk.GameStart, {
  ...
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill(phaavshsfec.name) then
      local catapult = table.find(U.prepareDeriveCards(player.room, {{ "phaavshsfec_phaavs", Card.Heart, 8 }}, phaavshsfec.name), function (id)
  ...
  on_use = function(self, event, target, player, data)
    ...
    local catapult = table.find(U.prepareDeriveCards(room, {{"phaavshsfec_phaavs", Card.Diamond, 9}}, phaavshsfec.name), function (id)
      return player.room:getCardArea(id) == Card.Void
    end)
    if catapult then
  ...
  can_trigger = function(self, event, target, player, data)
    if target == player and player:hasSkill(phaavshsfec.name) then
      if not table.find(player:getEquipments(Card.SubtypeTreasure), function(id)
        return Fk:getCardById(id).name == "phaavshsfec_phaavs"
      end) then

        local catapult = table.find(U.prepareDeriveCards(player.room, {{"phaavshsfec_phaavs", Card.Diamond, 9}}, phaavshsfec.name), function (id)
          return player.room:getCardArea(id) == Card.Void
        end)
        return catapult and player:canMoveCardIntoEquip(catapult)
      end
