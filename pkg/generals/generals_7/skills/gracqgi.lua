...
gracqgi:addEffect(fk.GameStart, {
  ...
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill(gracqgi.name) then
      local catapult = table.find(U.prepareDeriveCards(player.room, {{ "gracqgi_gi", Card.Heart, 8 }}, gracqgi.name), function (id)
  ...
  on_use = function(self, event, target, player, data)
    ...
    local catapult = table.find(U.prepareDeriveCards(room, {{"gracqgi_gi", Card.Diamond, 9}}, gracqgi.name), function (id)
      return player.room:getCardArea(id) == Card.Void
    end)
    if catapult then
  ...
  can_trigger = function(self, event, target, player, data)
    if target == player and player:hasSkill(gracqgi.name) then
      if not table.find(player:getEquipments(Card.SubtypeTreasure), function(id)
        return Fk:getCardById(id).name == "gracqgi_gi"
      end) then

        local catapult = table.find(U.prepareDeriveCards(player.room, {{"gracqgi_gi", Card.Diamond, 9}}, gracqgi.name), function (id)
          return player.room:getCardArea(id) == Card.Void
        end)
        return catapult and player:canMoveCardIntoEquip(catapult)
      end
