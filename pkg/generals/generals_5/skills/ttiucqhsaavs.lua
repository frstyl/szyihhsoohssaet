  ...
  can_trigger = function(self, event, target, player, data)
    if not player:hasSkill(ttiucqhsaavs.name) then return end
      ...
      for _, move in ipairs(data) do
        if move.to == player and table.contains({Card.PlayerHand }, move.toArea) and move.skillName ~= ttiucqhsaavs.name  then

          for _, info in ipairs(move.moveInfo) do
            if not (move.from==player and table.contains({Card.PlayerHand }, info.fromArea) ) then
              n=n+1
            end
          end
        end
