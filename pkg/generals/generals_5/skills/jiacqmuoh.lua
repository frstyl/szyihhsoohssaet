...
jiacqmuoh:addEffect(fk.CardUsing, {----TargetSpecified
  ...
  can_trigger = function(self, event, target, player, data)
    return data.from == player and player:hasSkill(jiacqmuoh.name) and data.card.trueName=="ssaet"
  end,
...
jiacqmuoh:addEffect(fk.DamageCaused, {  --不算發動技能
  ...
  anim_type = "offensive",
  can_trigger = function(self, event, target, player, data)
    if player.seat==1 and player.room.logic:damageByCardEffect() then  --目幖
