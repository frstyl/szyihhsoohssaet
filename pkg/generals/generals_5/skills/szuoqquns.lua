...
Fk:loadTranslationTable{
  ["szuoqquns"] = "輸運",
  [":szuoqquns"] = "➀補段終旹,若伱无資,伱可發動.伱改爲亮出牌堆頂x牌,伱選其中0至多類花不全同者各1置于伱武將牌上,稱爲資,餘者置入弃牌堆➁一脚色A主段始旹,若伱有資伱可發動.當段內,該A起動打出牌旹,若其類或花与第一張資相同,A取得該資.",  --淸理?
...
local spec = {
  can_trigger = function(self, event, target, player, data)
    ...
    and (S.compareCardType(data.card,player:getPile("szuoqquns_tsji")[1]) or data.card.suit==Fk:getCardById(player:getPile("szuoqquns_tsji")[1]).suit )
  end,
