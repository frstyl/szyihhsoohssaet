...
Fk:loadTranslationTable{
  ["biukhsooh"] = "伏虎",
  [":biukhsooh"] = "一其它脚色起動【殺】致傷後或被抵消後,，(若其未陣亾)伱可將1{♠️/♠️/无花}牌轉化爲殺對其預起動發動,若伏虎殺:元牌爲酒或裝僃,其傷害基數+1;結算後未致傷,伱抽x(x爲其傷害基數)",

  ["#biukhsooh-use"] = "伏虎 將非紅牌轉化爲殺對 %src  起動",
...
local spec ={
  ...
  can_trigger = function(self, event, target, player, data)
    return data.from~=player and  player:hasSkill(biukhsooh.name) 
  ...
  on_use = function(self, event, target, player, data)
    ...
    if not player:canUseTo(use.card,data.from,{ bypass_distances = true,bypass_times = true}) then return end--无視合理
    room:useCard(use)
