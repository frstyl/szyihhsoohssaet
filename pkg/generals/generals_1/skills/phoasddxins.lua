...
phoasddxins:addEffect(fk.CardUsing, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target==player and  player:hasSkill(phoasddxins.name) and S.isInstantTrick(data.card.trueName)
  end,
  ...
  on_refresh = function(self, event, target, player, data)
    ...
    if  target==player and suit == player:getMark("@phoasddxins") and S.isInstantTrick(data.card.trueName) then
