...
Fk:loadTranslationTable{
  ["gxeqprac"] = "奇兵",
  [":gxeqprac"] = "一脚色A成爲B所起動進攻牌C幖後(每次起動限1次),若伱至A距離不大于1且B不爲伱,伱可將1牌轉化爲C對B起動發動.",
...
--   end,
-- }
gxeqprac:addEffect(fk.TargetConfirmed, {
  anim_type = "offensive",
  can_trigger = function(self, event, target, player, data)
    ...
    -- and data:isOnlyTarget(data.to)
	    and not (data.extra_data and data.extra_data.gxeqprac)
  ...
  on_use = function(self, event, target, player, data)
    ...
    data.extra_data.gxeqprac=player.id
